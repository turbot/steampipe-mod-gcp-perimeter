...
control "cross_project_service_account_use" {
  ...
  documentation = file("./perimeter/docs/cross_project_service_account_use.md")
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        when service_account_email is not null then 'Service account is used across projects'
        else 'Service account is used within its own project'
      end as reason,
      project_id
...
control "vpc_shared_outside_org" {
  ...
  documentation = file("./perimeter/docs/vpc_shared_outside_org.md")
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        when shared_project_id is not null then 'VPC network is shared with untrusted project: ' || shared_project_id
        else 'VPC network is not shared with untrusted projects'
      end as reason,
      project_id
      ${local.tag_dimensions_sql}
    from
...
control "kms_key_shared_outside_org" {
  ...
  documentation = file("./perimeter/docs/kms_key_shared_outside_org.md")
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        when bound_project_id not in (select p from unnest($1::text[]) as p) then 'KMS key is shared with untrusted project: ' || bound_project_id
        else 'KMS key is only shared with trusted projects'
      end as reason,
      project_id
      ${local.tag_dimensions_sql}
    from
